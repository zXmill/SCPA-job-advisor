...
LOCAL_SKILL_ALIASES: dict[str, set[str]] = {
    ...
    "Web": {"web", "web applications", "web development"},
    ...
    "Data Analysis": {"analisis data", "analisa data", "data insights", "data insight"},
    "Data Science": {"ilmu data"},
    "Data Engineering": {"rekayasa data"},
    "Training": {"training", "pelatihan", "training execution", "training program", "training programs"},
    "Onboarding": {"onboarding", "orientasi", "mitra onboarding"},
    "Retention": {"retention", "retensi", "mitra retention"},
    "Operations": {"operations", "operational", "operasional", "operational alignment"},
    "Quality Assurance": {"quality assurance", "qa", "jaminan kualitas"},
    "Reporting": {"reporting", "reports", "laporan", "performance reports"},
    "Performance Monitoring": {"performance monitoring", "monitoring metrics", "track metrics"},
    "Program Management": {"program management", "program supervisor", "program coordination"},
...
def load_skill_seed() -> tuple[dict[str, Any], ...]:
    ...
    for name, aliases in {**LOCAL_SKILL_ALIASES, **file_aliases}.items():
        ...
        if canonical in LOCAL_SKILL_CATEGORIES:
            existing["category"] = LOCAL_SKILL_CATEGORIES[canonical]
        existing["source"] = "+".join(
