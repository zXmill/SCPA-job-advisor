...
_ESTIMATED_JOBS_PER_SEED = max(1, int(os.getenv("SCRAPER_ESTIMATED_JOBS_PER_SEED", "12")))
_MIN_ROUNDS_PER_SOURCE = max(1, int(os.getenv("SCRAPER_MIN_ROUNDS_PER_SOURCE", "2")))
...
def _select_runtime_seed_urls(seed_urls: list[SeedUrl], limit: int, seed_offset: int = 0) -> list[SeedUrl]:
    """Select seeds fairly across sources so one large source cannot starve others."""
    if limit <= 0 or len(seed_urls) <= limit:
        return seed_urls[:]

    grouped: dict[str, list[SeedUrl]] = {}
    for seed in seed_urls:
        grouped.setdefault(seed.source, []).append(seed)
    if seed_offset > 0:
        for source, bucket in list(grouped.items()):
            if not bucket:
                continue
            offset = seed_offset % len(bucket)
            grouped[source] = [*bucket[offset:], *bucket[:offset]]

    selected: list[SeedUrl] = []
    ordered_sources = sorted(grouped, key=_source_priority_key)
...
def _runtime_seed_budget(seed_urls: list[SeedUrl], raw_target: int, max_urls: int) -> int:
    source_count = len({seed.source for seed in seed_urls}) or 1
    target_budget = (max(1, raw_target) + _ESTIMATED_JOBS_PER_SEED - 1) // _ESTIMATED_JOBS_PER_SEED
    source_coverage_budget = source_count * _MIN_ROUNDS_PER_SOURCE
    return min(max_urls, _RUNTIME_URL_CAP, max(source_coverage_budget, target_budget))
    ...
    if isinstance(value, list):
        for item in value:
            ...
            if isinstance(item, dict):
                text = _first_value(item, ["name", "title", "label", "value", "skill"])
                if not text:
                    nested_skill = item.get("skill")
                    if isinstance(nested_skill, dict):
                        text = _first_value(nested_skill, ["name", "title", "label", "value"])
            else:
...
def _merge_skill_signals(*skill_lists: list[str]) -> list[str]:
    merged: list[str] = []
    seen: set[str] = set()
...
def _location_value(raw: dict[str, Any]) -> str:
    hierarchical_location = raw.get("location")
    if isinstance(hierarchical_location, dict):
        names = [
            clean_text(
                str(
                    hierarchical_location.get(key)
                    or ""
                )
            )
            for key in ("formattedName", "name")
        ]
        parents = hierarchical_location.get("parents")
        if isinstance(parents, list):
            for parent in parents:
                if isinstance(parent, dict):
                    names.append(clean_text(str(parent.get("formattedName") or parent.get("name") or "")))
        deduped = []
        seen = set()
        for name in names:
            key = name.lower()
...
def _source_url_value(raw: dict[str, Any], source_url: str | None) -> str:
    ...
    if source == "glints" and job_id:
        title_slug = re.sub(r"[^a-z0-9]+", "-", clean_text(str(raw.get("title") or "")).lower()).strip("-")
        suffix = f"/{title_slug}" if title_slug else ""
        return f"https://glints.com/id/opportunities/jobs/{job_id}{suffix}"

    return source_url or ""


def _description_from_json(raw: dict[str, Any], tags: list[str]) -> str:
    ...
    parts = [
        _first_value(raw, ["title", "job_title", "position", "role", "name"]),
        _first_value(raw, ["company_name", "companyName", "company", "organization", "employer", "hiring_company"]),
        _location_value(raw),
    ]
    category = raw.get("hierarchicalJobCategory")
    if isinstance(category, dict):
        parts.append(_first_value(category, ["name", "title"]))
        parents = category.get("parents")
        if isinstance(parents, list):
            parts.extend(_first_value(parent, ["name", "title"]) for parent in parents if isinstance(parent, dict))
    work_arrangement = _first_value(raw, ["workArrangementOption", "type", "educationLevel"])
    if work_arrangement:
        parts.append(work_arrangement.replace("_", " ").title())
    if tags:
        parts.append(
            "Source-listed skill requirements and matching signals: "
            + ", ".join(tags)
            + ". These source-listed skills are used as structured recommendation signals"
        )
    experience_parts = []
    min_exp = raw.get("minYearsOfExperience")
    max_exp = raw.get("maxYearsOfExperience")
    if min_exp is not None:
        experience_parts.append(f"minimum {min_exp} years")
    if max_exp is not None:
        experience_parts.append(f"maximum {max_exp} years")
    if experience_parts:
        parts.append("Experience metadata from the source: " + ", ".join(experience_parts))
    if len(" ".join(parts)) < _SCRAPE_QUALITY_MIN_DESC_CHARS and parts:
        parts.append(
            "The source listing did not expose a full public description in the search payload, "
            "so this record preserves the real structured title, company, location, category, "
            "salary, seniority, and skill metadata for matching until the detail page can be enriched."
        )
    return clean_text(". ".join(part for part in parts if clean_text(str(part or ""))))
...
def _job_item(
    *,
    ...
    if not required_skills:
        required_skills = _infer_skills(
            title,
            parsed.description_sections.get("role_overview", ""),
            parsed.description_sections.get("responsibilities", ""),
    ...
    extracted_skills = _merge_skill_signals(_infer_skills(title, description_text, " ".join(tags)), tags)
    skills = _merge_skill_signals(required_skills, preferred_skills, extracted_skills) or tags
    return JobItem(
        job_id=digest,
        title=title,
    ...
    if isinstance(salaries, list):
        for item in salaries:
            if not isinstance(item, dict):
                continue
            currency = clean_text(str(item.get("CurrencyCode") or item.get("currency") or ""))
            min_amount = item.get("minAmount") or item.get("min")
            max_amount = item.get("maxAmount") or item.get("max")
            mode = clean_text(str(item.get("salaryMode") or item.get("mode") or "")).lower()
            if min_amount and max_amount:
                return f"{currency} {min_amount} - {max_amount}{(' / ' + mode) if mode else ''}".strip()
            amount = min_amount or max_amount
            if amount:
                return f"{currency} {amount}{(' / ' + mode) if mode else ''}".strip()
    return None
...
def extract_jobs_from_json(payload: Any, source_url: str | None = None, limit: int = 25) -> ScrapeResponse:
    ...
    for raw in _records_from_json(payload):
        ...
        description = _description_from_json(raw, tags)
...
async def _enrich_job_detail(client: httpx.AsyncClient, job: JobItem) -> JobItem:
    ...
    update = job.model_copy(
        update={
            ...
            "preferred_skills": preferred_skills,
            "extracted_skills": extracted_skills,
            "skills": _merge_skill_signals(required_skills, preferred_skills, extracted_skills) or job.tags,
            "salary_text": salary,
        }
    )
    return update
...
async def scrape_run(limit: int = 100, seed_offset: int = 0):
    ...
    selected_urls = _select_runtime_seed_urls(
        seed_urls,
        _runtime_seed_budget(seed_urls, raw_target, max_urls),
        max(0, int(seed_offset or 0)),
    )
    ...
    async def fetch_seed(client: httpx.AsyncClient, seed: SeedUrl) -> tuple[SeedUrl, ScrapeResponse | None, str | None]:
        async with semaphore:
            try:
                ...
                return seed, None, (
                    f"source fetch or parse exceeded "
                    f"{_SEED_TASK_TIMEOUT_SECONDS:.0f}s/{_SEED_PARSE_TIMEOUT_SECONDS:.0f}s"
                )
