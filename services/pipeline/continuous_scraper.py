...
DEFAULT_ALLOWED_SOURCES = ("kalibrr",)
...
class ContinuousScrapeConfig:
    ...
    def from_env(cls) -> "ContinuousScrapeConfig":
        ...
        allowed_sources = tuple(
            source.strip().lower()
            for source in os.getenv("SCRAPER_ALLOWED_SOURCES", "kalibrr").split(",")
            if source.strip()
        )
...
class ContinuousScrapeRunner:
    ...
    async def run(self) -> ContinuousRunSummary:
        ...
        while not self._stop_event.is_set():
            ...
            next_sleep = calculate_sleep_seconds(self.config, empty_streak)
            cycle = CycleSummary(
                cycle_number=cycle_number,
