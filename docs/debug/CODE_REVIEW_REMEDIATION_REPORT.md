...
Recovery validation initially reopened R-2 because `013_hot_indexes_concurrent.py` changed isolation level inside Alembic's active transaction. The final fix uses Alembic `autocommit_block()` and also updates the original 009 hot-index migration so fresh deployments do not create these indexes non-concurrently before the repair migration runs.
...
| R-2 | P0 | `db/migrations/009_reco_hot_indexes.py`, `db/migrations/013_hot_indexes_concurrent.py` | Non-concurrent index creation blocks writes | Use `CREATE/DROP INDEX CONCURRENTLY` inside Alembic `autocommit_block()`; keep 013 as idempotent repair |
...
| `.\.venv\Scripts\python.exe -m alembic -c alembic.ini upgrade head` | pass after migration fix |
| `.\.venv\Scripts\python.exe -m alembic -c alembic.ini downgrade 012_ab_testing_and_monitoring` | pass |
| `.\.venv\Scripts\python.exe -m alembic -c alembic.ini upgrade head` | pass after downgrade smoke |
...
- Resume the frontend product-quality/data-quality phase only if the user asks to continue it.
- Keep P2 cleanup deferred until a separate scoped pass.
