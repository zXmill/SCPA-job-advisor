...
async def _insert_training_job_with_polluted_source_tag(
    db_session: AsyncSession,
    job_id: str,
) -> None:
    description = (
        "Role Overview The Hub Mitra Training & Program Supervisor ensures "
        "mitra onboarding, training execution, retention, operational alignment, "
        "quality assurance, reporting, performance monitoring, and improvement "
        "initiatives based on data insights."
    )
    await db_session.execute(
        text(
            "INSERT INTO jobs ("
            "id, title, company, location, type, salary_currency, posted_at, "
            "is_active, description_text, required_skill_names, extracted_skill_names, match_data"
            ") VALUES ("
            ":id, 'Hub Mitra Training & Program Supervisor', 'Astro', "
            "'West Jakarta, Indonesia', 'full_time', 'IDR', NOW(), true, "
            ":description_text, :required_skill_names, :extracted_skill_names, "
            "CAST(:match_data AS jsonb)"
            ")"
        ).bindparams(
            bindparam("required_skill_names", type_=ARRAY(Text())),
            bindparam("extracted_skill_names", type_=ARRAY(Text())),
        ),
        {
            "id": uuid.UUID(job_id),
            "description_text": description,
            "required_skill_names": ["E-Commerce"],
            "extracted_skill_names": ["Monitoring"],
            "match_data": json.dumps(
                {
                    "skills": ["E-Commerce"],
                    "required_skills": ["E-Commerce"],
...
async def test_skill_gap_detail_ignores_source_category_when_jd_text_lacks_evidence(
    client,
    db_session: AsyncSession,
) -> None:
    user = await _register(client, email="skill-gap-training@example.com")
    job_id = str(uuid.uuid4())
    await _insert_user_skill(db_session, user["user"]["id"], "Operations")
    await _insert_training_job_with_polluted_source_tag(db_session, job_id)
    ...
    assert response.status_code == 200, response.text
    body = response.json()
    assert "E-Commerce" not in body["required_skills"]
    assert {"Training", "Operations", "Reporting"} & set(body["required_skills"])
    assert "Operations" in body["matched_skills"]
