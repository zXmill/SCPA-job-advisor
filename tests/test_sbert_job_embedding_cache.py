...
def test_job_embedding_text_includes_structured_description_and_skill_signals() -> None:
    text = stage_2_encode._job_text(
        {
            **_job(),
            "description": "Short card copy.",
            "description_text": "Full detail page copy.",
            "responsibilities": ["Prepare weekly performance reports."],
            "requirements": ["Training execution experience."],
            "required_skills": ["Training", "Reporting"],
            "extracted_skills": ["Performance Monitoring"],
        }
    )

    assert "Full detail page copy." in text
    assert "Prepare weekly performance reports." in text
    assert "Training execution experience." in text
    assert "Performance Monitoring" in text
