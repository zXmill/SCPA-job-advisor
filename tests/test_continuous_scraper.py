...
from services.pipeline.continuous_scraper import (
    ...
    QualityGuardSummary,
    calculate_effective_cycle_limit,
    calculate_next_sleep_seconds,
    calculate_seed_offset,
...
def test_catch_up_interval_until_active_job_target_is_reached() -> None:
    config = ContinuousScrapeConfig(
        cycle_interval_seconds=900,
        run_forever=True,
        active_job_target=1000,
        catch_up_interval_seconds=60,
    )

    assert calculate_next_sleep_seconds(config, empty_cycle_streak=0, total_active_jobs=50) == 60
    assert calculate_next_sleep_seconds(config, empty_cycle_streak=0, total_active_jobs=1000) == 900


def test_catch_up_cycle_limit_expands_until_active_job_target_is_reached() -> None:
    config = ContinuousScrapeConfig(
        cycle_limit=250,
        run_forever=True,
        active_job_target=1000,
        catch_up_cycle_limit=1000,
    )

    assert calculate_effective_cycle_limit(config, total_active_jobs=361) == 639
    assert calculate_effective_cycle_limit(config, total_active_jobs=999) == 250
    assert calculate_effective_cycle_limit(config, total_active_jobs=1000) == 250


def test_seed_offset_advances_between_run_forever_cycles() -> None:
    config = ContinuousScrapeConfig(seed_rotation_stride=6)

    assert calculate_seed_offset(config, cycle_number=1) == 0
    assert calculate_seed_offset(config, cycle_number=2) == 6
    assert calculate_seed_offset(config, cycle_number=5) == 24


def test_continuous_scraper_defaults_allow_all_enabled_job_sources(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("SCRAPER_ALLOWED_SOURCES", raising=False)
    ...
    assert {
        ...
        "techinasia",
        "indeed",
    }.issubset(set(config.allowed_sources))
