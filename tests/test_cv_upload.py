...
async def _insert_ai_noise_skills(db_session: AsyncSession) -> None:
    skills = [
        ("AI Squared ZoomText", "tool", ["ai"], "O*NET 30.3 Software Skills", 0.88),
        ("Artificial Intelligence", "technical", ["ai"], "local_alias", 1.0),
    ]
    for name, category, aliases, source, confidence in skills:
        await db_session.execute(
            text(
                "INSERT INTO skills (name, category, aliases, source, confidence) "
                "VALUES (:name, :category, :aliases, :source, :confidence) "
...
async def test_cv_upload_filters_obscure_software_taxonomy_noise(client, db_session) -> None:
    await _insert_ai_noise_skills(db_session)
    reg = await _register(client, email="cv-ai-user@example.com")
    ...
    response = await client.post(
        ...
        files={
            "file": (
                "cv_ai.txt",
                b"AI product analyst with stakeholder reporting experience.",
                "text/plain",
            )
        },
    )
    ...
    assert response.status_code == 200, response.text
    data = response.json()
    assert "Artificial Intelligence" in data["extracted_skills"]
    assert "AI Squared ZoomText" not in data["extracted_skills"]
